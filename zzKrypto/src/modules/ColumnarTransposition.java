...
public class ColumnarTransposition {
    private static List <com.mPair<Character, Integer>> prepareKey (String input) {
        List <com.mPair<Character, Integer>> key = new ArrayList<>();
        String preKey = removeDuplicates(input); //replace with getString()

        for (int i = 0; i < preKey.length(); ++i) {
            char c = preKey.charAt(i);
            com.mPair<Character, Integer> temp = new com.mPair<Character, Integer>(c, i);
            key.add(temp);
        }
    ...
    public static List<List<Character>> encrypt (String plaintext, String preKey, List<List <Character>> cipher) {
        ...
        List <com.mPair<Character, Integer>> key = prepareKey(preKey);
    ...
    private static List<List<Character>> mixColumns (List<List<Character>> input,
                                                     List <com.mPair<Character, Integer>> key) {
        ...
        for (com.mPair <Character, Integer> k: key) {
