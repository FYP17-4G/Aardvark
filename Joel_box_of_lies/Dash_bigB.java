...
class Dash_bigB {
    ...
    Dash_bigB(String input_key) {

        System.out.println("Dash_bigB() called!"); //@debug
    ...
    void transformText() {
        ...
        for(int y = 0; y < modifiedText.length(); y++ ) {
            ...
            int PT = modifiedText.charAt(y) - 'A';              //int value of y-th input character a
            int k = key.charAt( mod(y, key.length())) - 'A';    //int value of y-th (mod keylength) input character
